...
async function createConsultationLead(req, res) {
  ...
  const { name, phone, email, date, description } = req.body;

  if (!name || !phone || !email || !date || !description) {
  ...
  try {
    ...
    res.status(201).json({
      message: `<div class="col-span-2 text-center"><h1 class="text-2xl font-semibold">✅ 1-on-1 booked successfully for ${date}!</h1><br/><h2 class="text-xl">An email has been sent to ${email}.</h2><button
            id="continueButton"
